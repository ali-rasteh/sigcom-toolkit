...
class Plot_Utils(Signal_Utils):
    ...
    def set_plot_params(ax=None, lines=None, plot_params_dict=None):
        ...
        if plot_params_dict["title"] is not None:
            ...
            title = (
                (title[: plot_params_dict["title_max_chars"]] + "...")
                if len(title) > plot_params_dict["title_max_chars"]
                else title
            )
        ...
        ax.tick_params(
            axis="both", which="major", labelsize=plot_params_dict["ticks_size"]
        )  # For major ticks
        ...
        if plot_params_dict["hspace"] is not None and plot_params_dict["wspace"] is not None:
            plt.subplots_adjust(
                hspace=plot_params_dict["hspace"], wspace=plot_params_dict["wspace"]
            )
    ...
    def draw_half_gauge(ax, min_val=-90, max_val=90):
        ...
        for i in range(num_ticks + 1):
            ...
            ax.plot(
                [
                    0.5 + 0.35 * np.cos(np.radians(angle)),
                    0.5 + (0.35 - tick_length) * np.cos(np.radians(angle)),
                ],
                [
                    0.5 + 0.35 * np.sin(np.radians(angle)),
                    0.5 + (0.35 - tick_length) * np.sin(np.radians(angle)),
                ],
                color="black",
                lw=1,
                zorder=3,
