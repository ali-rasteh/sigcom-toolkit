...
class Sim(Signal_Utils):
    ...
    def gen_tx_pos(self):
        ...
        if self.tx is None:
            ...
            self.tx = (
                np.random.uniform(0, 1, (self.npath, self.p)) * wid[None, :]
                + self.region[None, :, 0]
            )
    ...
    def compute_freq_resp(self):
        ...
        # why?
        phase = np.exp(1j * 2 * np.pi * np.random.uniform(0, 1, (self.nmeas, self.npath)))
        ...
        dnom = 10 * dsamp
        self.distref = (
            np.mean(self.dist[:, :, 0], axis=0)
            - dnom
            + np.random.uniform(-0.5, 0.5, self.nmeas) * dsamp
        )
        ...
        self.chan_fd = (
            self.chan
            + np.random.normal(0, np.sqrt(wvar / 2), self.chan.shape)
    ...
    def plot_chan_td(self, indsamp=None, indrx=0, indmeas=0):
        ...
        plt.stem(
            path_samp1.flatten() / self.fsamp * 1e9,
            path_pow1.flatten(),
            "r-",
            bottom=ymin,
            basefmt=" ",
    ...
    def path_est_init(self):
        ...
        for i in range(self.nmeas):
            ...
            self.chan_fd_rot[:, :, i] = self.chan_fd[:, :, i] * np.exp(
                1j * 2 * np.pi * im * f / self.nfft
            )
    ...
    def locate_tx(self, npath_est=None):
        ...
        for ipath in range(npath_est):
            if ipath == 0:
                ...
                self.rho[:, ipath] = np.sum(
                    np.abs(np.sum(aresp[:, :, None] * dexp, axis=0)) ** 2, axis=0
                )
    ...
    def fit_coeffs(self, npath_fit=1):
        ...
        self.basis1 = self.basis.reshape(
            (self.nfft * self.nantrx, self.nmeas, npath_fit), order="F"
        )
        ...
        for i in range(self.nmeas):
            self.coeffs_est[i, :] = np.linalg.lstsq(
                self.basis1[:, i, :], chan_fd_rot1[:, i], rcond=None
            )[0]

        # Compute channel from the sum of the estimated paths
        self.chan_fd_paths = np.sum(self.coeffs_est[None, :, :] * self.basis1, axis=2)
        self.chan_fd_paths = self.chan_fd_paths.reshape(
            (self.nfft, self.nantrx, self.nmeas), order="F"
        )
    ...
    def nf_channel_param_est(
        ...
        tx_ant_vec = torch.tensor(tx_ant_vec, dtype=torch.float32, requires_grad=True).to(
            self.t_device
        )
        rx_ant_vec = torch.tensor(rx_ant_vec, dtype=torch.float32, requires_grad=True).to(
            self.t_device
        )
        trx_unit_vec = torch.tensor(trx_unit_vec, dtype=torch.float32, requires_grad=True).to(
            self.t_device
        )
        if path_delay is not None:
            path_delay = torch.tensor(path_delay, dtype=torch.float32, requires_grad=True).to(
                self.t_device
            )
        if path_gain is not None:
            path_gain = torch.tensor(path_gain, dtype=torch.complex64, requires_grad=True).to(
                self.t_device
            )
        ...
        for epoch in range(n_epochs):  # Number of epochs
            ...
            loss = criterion(
                torch.abs((output - H_gt)[:, :, txid, :]),
                torch.zeros_like(output[:, :, txid, :], dtype=torch.float32),
            )
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if epoch % (n_epochs // 20) == 0:
                print(
                    f"Epoch {epoch + 1}/{n_epochs}, Loss: {loss.item()}, LR: {scheduler.get_last_lr()}"
                )
                # print("alpha: ", self.nf_ch_model.alpha)
                # print("s: ", self.nf_ch_model.s)
        ...
        if ax is None:
            ...
            for i_path in range(self.npath_det):
                if i_path == 0:
                    ...
                    color = "cX"
                point = ax.plot(
                    self.tx_est[i_path, 0], self.tx_est[i_path, 1], color, markersize=15
                )  # Estimated TX
                self.points.append(point[0])
...
class NF_Channel_Model(nn.Module):
    def __init__(self, n_path=1, n_rx=2, n_tx=2, n_meas=1, fc=10e9, device="cpu"):
        ...
        self.alpha = nn.Parameter(
            0.1 * torch.randn(self.n_path), requires_grad=True
        )  # Rotation angles
        self.s = nn.Parameter(
            0.1 * torch.randn(self.n_path), requires_grad=True
        )  # parameter S, the parity of number of reflections

        self.path_gain = torch.complex(
            torch.randn(self.n_path, self.n_rx, self.n_tx, self.n_meas),
            torch.randn(self.n_path, self.n_rx, self.n_tx, self.n_meas),
        )  # parameter G, the gain of the path
        self.path_gain = nn.Parameter(self.path_gain)
        # self.path_gain = None

        self.path_delay = torch.randn(
            self.n_path, self.n_rx, self.n_tx, self.n_meas
        )  # parameter tau, the delay of the path
        ...
        for i in range(self.n_path):
            ...
            dist_vec = (
                ...
                - (
                    constants.c
                    * (path_delay[i])[:, :, :, None]
                    * (trx_unit_vec[i])[None, None, :, :]
                )
                - (torch.tanh(tanh_scale * self.s[i]) * rot_tx_ant_vec)[None, :, :, :]
            )
            dist = torch.linalg.norm(dist_vec, dim=-1)
            f = self.fc + freq
            H += (path_gain[i])[None, :, :, :] * torch.exp(
                1j * 2 * np.pi / constants.c * f[:, None, None, None] * dist[None, :, :, :]
            )
...
if __name__ == "__main__":
    ...
    sim = Sim(
        ...
        antsep=antsep,
        npath=npath,
        lossavg_dB=lossavg_dB,
        tx=xtx,
        npath_est=npath_est[1],
    ...
    for i in range(h.shape[-1]):
        ...
        sparse_est_params = signals_inst.sparse_est(
            ...
            npaths=signals_inst.nf_npath_max,
            nframe_avg=1,
            ndly=ndly,
            drange=signals_inst.sparse_ch_samp_range,
            cv=True,
            n_ignore=signals_inst.sparse_ch_n_ignore,
        )
    ...
    plt.stem(
        dly_est_[0, 0, 0] * signals_inst.fs_trx,
        peaks_[0, 0, 0],
        "g-",
        bottom=np.min(peaks_) - 100,
        basefmt=" ",
    )
    plt.show()
