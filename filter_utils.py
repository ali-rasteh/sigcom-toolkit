...
class Filter_Utils(SignalUtils):
    ...
    def wiener_fir_param(
        self, sig_id, sig_bw, sig_pwr, sig_cf, spatial_sig, snr, filter_order_pos, filter_order_neg
    ):
        ...
        Rxx_b1 = (
            Rxx_b1[:, None]
            * np.exp(2 * np.pi * 1j * sig_cf[:, None] * t[None, :])
            * np.sinc(sig_bw[:, None] * t[None, :])
        )
        Rxx_b2 = np.sum(Rxx_b1, axis=0)
        Rxx = scipy.linalg.toeplitz(
            Rxx_b2[filter_length - 1 :], Rxx_b2[:filter_length][::-1]
        ) + N0 * np.eye(filter_length)
        ...
        Ryx_b1 = (
            np.conj(spatial_sig[0, sig_id])
            * sig_pwr[sig_id]
            * sig_bw[sig_id]
            * np.exp(2 * np.pi * 1j * sig_cf[sig_id] * t)
            * np.sinc(sig_bw[sig_id] * t)
        )
        ...
        for i in range(filter_length):
            for j in range(filter_length):
                if j >= i:
                    ...
                    Rxx[i * N_in : (i + 1) * N_in, j * N_in : (j + 1) * N_in] = np.conj(
                        Rxx_t[:, :, i - j]
                    ).T
    ...
    def wiener_fir_vector_param(
        self, sig_bw, sig_pwr, sig_cf, spatial_sig, snr, filter_order_pos, filter_order_neg
    ):
        ...
        for k in range(N_out):
            Rxx_b1[k, :, :] = (
                np.outer(spatial_sig[:, k], np.conj(spatial_sig[:, k])) * sig_pwr[k] * sig_bw[k]
            )
        t = np.arange(-(filter_length - 1), filter_length)
        Rxx_b2 = np.exp(2 * np.pi * 1j * sig_cf[:, None] * t[None, :]) * np.sinc(
            sig_bw[:, None] * t[None, :]
        )
        ...
        for i in range(filter_length):
            for j in range(filter_length):
                t = j - i
                Rxx[i * N_in : (i + 1) * N_in, j * N_in : (j + 1) * N_in] = Rxx_b3[
                    :, :, t + filter_length - 1
        ...
        Ryx_b2 = np.exp(2 * np.pi * 1j * sig_cf[:, None] * t[None, :]) * np.sinc(
            sig_bw[:, None] * t[None, :]
        )
    ...
    def basis_fir_us(
        self, input, fil_base, t, freq, center_freq, iters, us_rate, plot_procedure=False
    ):
        """
    ...
    def basis_fir_ds_us(
        self, input, fil_base, t, freq, center_freq, iters, ds_rate, us_rate, plot_procedure=False
    ):
        """
            ...
            if plot_procedure:
                ...
                for idx in range(4):
                    ...
                    if idx == 0:
                        ...
                        index = 1
                        plt.title(
                            "Frequency spectrum of the first round downsampled filtered signal"
                        )
                    elif idx == 2:
                        ...
                        index = iters
                        plt.title(
                            "Frequency spectrum of the last round downsampled filtered signal"
                        )
    ...
    def wiener_filter_design(
        ...
        if self.n_r <= 1:
            ...
            for i in range(self.n_sig):
                for j in range(self.n_r):
                    if mode == "sigs":
                        ...
                        self.fil_wiener_single[i][j] = self.wiener_fir_param(
                            i,
                            sig_bw,
                            sig_psd,
                            sig_cf,
            ...
            if mode == "sigs":
                fil_wiener = self.wiener_fir_vector(
                    rx, sigs, self.wiener_order_pos, self.wiener_order_neg
                )
            elif mode == "params":
                fil_wiener = self.wiener_fir_vector_param(
                    sig_bw,
                    sig_psd,
                    sig_cf,
                    spatial_sig,
    ...
    def wiener_filter_apply(self, rx, sigs):
        ...
        for i in range(self.n_sig):
            ...
            self.print(
                f"Time delay between the signal and its Wiener filtered version for signal {i + 1}: {time_delay} samples",
                3,
            )
